...
struct home: View {
        ...
        ZStack{
            ...
            VStack {
                ...
                VStack{
                                    ...
                                    HStack {
                                        ForEach(0..<8) { j in
                                            Circle()
                                                ...
                                                .overlay(Image(feature[i][j].imageName)
                                                    .resizable()
                                                    .scaledToFit()
                                                    .frame(width: 120, height: 120)
                                                    .offset(x: -5, y: 5)
                    ...
                    HStack (spacing: 120){
                        Button(action: {computermode = false}, label: {
                            Image("backtomenu")
                                .resizable()
                                .scaledToFill()
    ...
    func SolidColor() {
        for he in heal {
            if feature[he.x][he.y].transparent == 0.3 {
                ...
                feature[he.x][he.y].color = Color.gray
            }
        }
        heal = [coordinate]()
    }
    ...
    func highlight(){
        for mu in enemy{
            feature[mu.x][mu.y].transparent = 0
            feature[mu.x][mu.y].color = Color.gray
        }
        enemy = [coordinate]()
    }
    ...
    func CheckSelect(i:Int, j:Int){
        ...
        if eat.count != 0{
            ...
            if feature[i][j].choose == true && feature[i][j].color == turncolor && search == false{
                ...
                if turncolor == Color.orange{
                 let ind = Int.random(in: 0...enemy.count - 1)
                    CheckSelect(i: enemy[ind].x, j: enemy[ind].y)
                }
            } else if feature[i][j].transparent == 0.3 && feature[i][j].color == turncolor{
                highlight()
                feature[i][j].transparent = 1
                feature[i][j].color = turncolor
                feature[exchanges.x][exchanges.y].color = Color.gray
                ...
                feature[(exchanges.x+i)/2][(exchanges.y + j)/2].color = Color.gray
                GameWin()
                ...
                if enemy.count == 0{
                    ...
                    if turncolor == Color.orange{
                        search = false
                        black_finished = false
                        CheckSelect(i: eat[0].x, j: eat[0].y)
                    }
                }
            }
            ...
            if turncolor == Color.orange{
                ...
                if heal.count != 0{
                    let ind = Int.random(in: 0...heal.count - 1)
                    
                    CheckSelect(i: heal[ind].x, j: heal[ind].y)
                }
            } else{
                if feature[i][j].mark == false{
                    if i - 1 >= 0 && (j - 1 >= 0 || j + 1 < 8){
...
struct PvCView_Previews: PreviewProvider {
    static var previews: some View {
        home(computermode: .constant(true), present: .constant(.pve))
    }
}
